...
impl<'i> Statement<'i> {
    ...
    pub fn check_reachability_of_used_symbols(&self) -> Result<String, Error<Rule>> {
        ...
        if used_symbols.is_subset(&self.visible_symbols) {
            ...
            Err(self
                .make_semantic_error(format!("Unreachable symbols: {}", list_of_missing_symbols)))
        }
    }
}
    ...
    fn parse_expression(pair: Pair<Rule>) -> Expression {
        ...
        let inner = match pair.as_rule() {
            Rule::assignment => {
                ...
                let id: char = inner_pair.next().unwrap().as_str().chars().next().unwrap();
    ...
    fn parse_statement(pair: Pair<Rule>) -> Statement {
        ...
        let inner = match pair.as_rule() {
            ...
            Rule::scoped_statement => {
                let inner_statements = pair.into_inner().map(parse_statement).collect();
            ...
            Rule::function_declaration_statement => {
                ...
                let param_list: Vec<Identifier> = inner_pieces
                    .next()
                    .unwrap()
                    .into_inner()
            ...
            Rule::function_usage_statement => {
                ...
                let param_list: Vec<Expression> = inner_pieces
                    .next()
                    .unwrap()
                    .into_inner()
...
fn get_fn_name_for_current_scope(s: &str) -> String {
    unsafe { get_fn_name_for_scope(s, SCOPE_COUNTER) }
}
...
impl<'a, 'i> Compiler<'a, 'i> {
    ...
    fn compile_term(
        ...
        match (*term).inner {
            ...
            InnerTerm::Id(ref id) => {
                ...
                let loaded_local_variable = self
                    .builder
                    .build_load(*ptr_to_local_var, &format!("deref_{}", var_name));
    ...
    fn compile_expr(
        ...
        match (*expr).inner {
            ...
            // Parenthesis(Box<Expression<'i>>),
            InnerExpression::Parenthesis(ref inner_expr) => {
                self.compile_expr(inner_expr, local_vars)
            }
    ...
    fn compile_stmt(
        ...
        match (*stmt).inner {
            ...
            // Expr(Expression<'i>),
            InnerStatement::Expr(ref expr) => match self.compile_expr(expr, local_vars) {
            ...
            InnerStatement::Scope(ref stmts) => {
                ...
                for stmt in stmts {
                    if let InnerStatement::FnDeclarationStatement(ref s, _, _) = stmt.inner {
                        function_names_in_scope
                            .insert(s.to_string(), get_fn_name_for_current_scope(s));
                    }
                }
            ...
            InnerStatement::PrintStatement(ref printable) => {
                ...
                match printable.inner {
                    InnerPrintable::Str(ref s) => {
                        let static_str =
                            builder.build_global_string_ptr(&format!("{}\n", s), "static_s");

                        let static_str =
                            BasicValueEnum::PointerValue(static_str.as_pointer_value());
            ...
            InnerStatement::FnUsageStatement(ref name, ref exprs) => {
                ...
                for expr in exprs {
                    ...
                    let final_pointer_value =
                        BasicValueEnum::PointerValue(pointer_to_parameter.unwrap());
    ...
    fn compile_fn(
        ...
        let this_function_s_name = reachable_functions
            .get(&name.to_string())
        ...
        */
        ...
        for (idx, parameter_pointer) in function.get_params().iter().enumerate() {
            ...
            let loaded_parameter_value =
                builder.build_load(*parameter_pointer, &format!("deref_{}", var_name));
        ...
        */
        ...
        for (idx, parameter_pointer) in function.get_params().iter().enumerate() {
            ...
            let loaded_parameter_value =
                builder.build_load(ptr_to_local_var, &format!("deref_back_to_arg_{}", var_name));
